...
from pyrit.excitation import CurrentDensity, SolidConductor, FieldCircuitCoupling
...
# Variable to switch between one conductor and L_mat calculation.
calc_L_mat = False
...
def simulate_mqs(frequency: float, conductor: int, plots: bool):
    ...
    vals[conductor] = 1
    excis_left = [SolidConductor(current=val) for val in vals]  # List of excitations for the left side in the slot
    excis_right = [SolidConductor(current=val) for val in vals]  # List of excitations for the right side in the slot
    ...
    load = shape_function.load_vector(0) # problem.regions, problem.excitations

    # matrix_shrink, rhs_shrink, _, _, support_data = shape_function.shrink(matrix, load, problem, 1)
    matrix_shrink, rhs_shrink = FieldCircuitCoupling.shrink(matrix, load, problem, integration_order=1)
    a_shrink, _ = type(problem).solve_linear_system(matrix_shrink.tocsr(), rhs_shrink.tocsr(), solver='spsolve')
    ...
    if plots:
        ...
        mesh.plot_equilines(np.real(vector_potential), title="vector potential equilines")

        plt.show()
    # endregion
    return vector_potential, b_field
...
def main():
    ...
    if calc_L_mat:
        i = 0
        L_mat = np.zeros([18, 18])

        # loop over excitation of each conductor pair
        for i in range(18):
            print(i)
        # Induction law U = L * jw * I -> L_ab = U_b / (jw) with I_a == 1
        # need more info about voltages ...

    else:
        # calc mqs for excit. on cond. 13 with f = 50 Hz
        vector_pot, b_field = simulate_mqs(50, 11, True)
